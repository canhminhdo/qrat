...
bool PrismRunner::isAvailable() {
    ...
    if (prismPath == nullptr) {
        std::cerr << Tty(Tty::RED) << "Error: " << Tty(Tty::RESET) << "Environment variable PRISM_PATH is not set." << std::endl;
        return false;
    }
    // Check if the prism model checker exists and executable
    if (access(getPath().c_str(), F_OK) == -1) {
        std::cerr << Tty(Tty::RED) << "Error: " << Tty(Tty::RESET) << "Could not find the PRISM model checker in PRISM_PATH." << std::endl;
        return false;
    }
    return true;
}
